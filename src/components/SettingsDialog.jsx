...
import { Bot, Settings, AlertTriangle, Info } from 'lucide-react';

export function SettingsDialog({ open, onOpenChange }) {
    const { settings, aiAvailable, updateSettings, availableModels } = useSettings();
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[425px]">
                ...
                <div className="grid gap-6 py-4">
                    {/* AI Section */}
                    {!aiAvailable ? (
                        <div className="flex gap-2 items-start p-3 bg-muted border rounded-md text-muted-foreground text-sm">
                            <Info size={16} className="mt-0.5 shrink-0" />
                            <span>
                                AI features are not available. To enable them, set <code className="bg-background px-1 rounded">OPENROUTER_API_KEY</code> in your server environment.
                            </span>
                        </div>
                    ) : (
                        <>
